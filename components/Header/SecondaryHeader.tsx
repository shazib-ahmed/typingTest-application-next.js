...
import DesktopMenu2 from "./Headercomp/DesktopMenu2";
import MobileMenu2 from "./Headercomp/MobileMenu2";

const addClass = (ref: any, myclass: string) => {
  ref.current?.classLIst.add(myclass);
};
const SecondaryHeader = (props: { finishedLoading: boolean; sectionsRef }) => {
  ...
  return (
    <>
      ...
      justify-between px-6 sm:px-12 py-2 sm:py-4 transition duration-4000 z-20`}
      >
        {/* Logo S */}
        ...
        <DesktopMenu2 finishedLoading={props.finishedLoading} />
      </motion.div>
    </>
  );
};
